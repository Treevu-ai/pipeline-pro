...
def generate_referral_code(phone: str, max_referrals: int = 10) -> str:
    ...
    try:
        with _conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO referral_codes (code, phone, max_referrals, used_count, created_at, expires_at)
                    VALUES (%s, %s, %s, 0, now(), now() + INTERVAL '90 days')
                    ON CONFLICT (code) DO UPDATE
                        SET phone = EXCLUDED.phone,
                            max_referrals = EXCLUDED.max_referrals,
                            used_count = 0,
                            created_at = now()
                """, (code, phone, max_referrals))
        return code
    except Exception as exc:
        log.error("generate_referral_code(%s): %s", phone, exc)
        return code


def get_referral_code(code: str) -> dict | None:
    """Obtiene info de un código de referido."""
    if not _USE_DB:
        return None
    try:
        with _conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT code, phone, max_referrals, used_count, created_at, expires_at
                    FROM referral_codes
        ...
        log.error("get_referral_code(%s): %s", code, exc)
        return None


def use_referral_code(code: str, new_phone: str) -> bool:
    """Registra uso del código de referido."""
    if not _USE_DB:
        return False
    try:
        with _conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    WHERE code = %s AND used_count < max_referrals AND expires_at > now()
                    RETURNING phone
                """, (code,))
                row = cur.fetchone()
                if not row:
                ...
                cur.execute("""
                    INSERT INTO referral_rewards (phone, referred_by, reward_plan, reward_days, status)
                    VALUES (%s, %s, 'trial', 30, 'pending')
                """, (new_phone, referrer_phone))
        return True
    except Exception as exc:
        log.error("use_referral_code(%s -> %s): %s", code, new_phone, exc)
        return False


def get_user_referrals(phone: str) -> list[dict]:
    """Lista de referidos por un usuario."""
    if not _USE_DB:
        return []
    try:
        with _conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT phone, reward_plan, status, created_at, activated_at
        ...
        log.error("get_user_referrals(%s): %s", phone, exc)
        return []


def activate_referralReward(phone: str, plan: str) -> dict | None:
    """Activa reward cuando el referido hace upgrade."""
    if not _USE_DB:
        return None
    try:
        reward_days = 30 if plan in ("basico", "starter") else 60 if plan == "pro" else 90
        ...
        with _conn() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    RETURNING referred_by, reward_days
                """, (plan, reward_days, phone))
                row = cur.fetchone()
                if not row:
                    return None
                referrer_phone, days = row
                
                upsert_subscriber(referrer_phone, plan="trial", days=days)
                return {"referred_by": referrer_phone, "days": days}
    except Exception as exc:
        log.error("activate_referralReward(%s): %s", phone, exc)
        return None
...
def get_referral_stats() -> dict:
    ...
    try:
        with _conn() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT COUNT(*) FROM referral_codes WHERE expires_at > now()")
                active_codes = (cur.fetchone() or [0])[0]
                
                cur.execute("SELECT COUNT(*) FROM referral_rewards")
                total_referrals = (cur.fetchone() or [0])[0]
                
                cur.execute("SELECT COUNT(*) FROM referral_rewards WHERE status = 'active'")
                active_rewards = (cur.fetchone() or [0])[0]
                
                return {
                    "active_codes": active_codes,
                    "total_referrals": total_referrals,
                    "active_rewards": active_rewards,
                    "conversion_rate": f"{round(active_rewards/total_referrals*100, 1)}%" if total_referrals else "0%",
                }
    except Exception as exc:
        log.error("get_referral_stats: %s", exc)
        return {}
