...
_KEYWORDS: dict[str, list[str]] = {
    ...
    "upgrade":   ["upgrade", "plan completo", "quiero plan", "starter", "acceso", "comprar", "🚀 quiero", "activar", "activar plan",
                  "sí, quiero", "si, quiero", "sí quiero", "si quiero", "quiero configurar", "configurar todo"],
...
def _handle_message_locked(phone: str, text: str) -> list[dict]:
    ...
    # ── Mapeo directo de opciones del menú ───────────────────────────
    # Soporta números (1,2,3) Y letras (A/B/C o a/b/c) cuando el menú está activo
    if state in ("menu_shown", "idle", "done", "post_pdf_options"):
        t = text.strip().lower()
        # Mapeo: 1->demo, 2->precios, 3->info / a->demo, b->precios, c->info
        _menu_map = {
            "1": "demo", "2": "precios", "3": "info",
            "a": "demo", "b": "precios", "c": "info",
        }
        if t in _menu_map:
            intent = _menu_map[t]
