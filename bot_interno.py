...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        "🤖 *PipeAssist — Comandos*\n\n"
        "*Pipeline:*\n"
        "`/pipeline <query>` — Scrape + calificación → CSV\n"
        "`/buscar <query>` — Solo scrape, sin LLM\n\n"
        "*Explorar resultados:*\n"
        "`/historial [N]` — Últimas N ejecuciones\n"
        "`/top [N]` — Top N leads por score\n"
        "`/calificados` — Leads etapa Calificado\n"
        "`/seguimiento` — Leads En seguimiento\n"
        "`/stats [run_id]` — Estadísticas del run\n"
        "`/csv [run_id]` — Descargar CSV\n\n"
        "*Sistema:*\n"
        "`/status` — Estado: API keys, último run\n"
        "`/test` — Test del LLM\n\n"
        "*Flags:* `--limit N` · `--channel email|whatsapp|both`\n\n"
        "💬 Escribe texto libre para preguntarle al asistente."
    )
