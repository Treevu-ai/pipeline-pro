    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            if not resp.is_success:
                ...
                body_snippet = utils.trunc(resp.text, 2000)
                relevant_headers = {
                    k: v for k, v in resp.headers.items()
                    if k.lower() in ("content-type", "x-request-id", "retry-after", "x-apify-error-type")
                }
                if status == 401:
                    log.error(
                        "Apify HTTP 401 Unauthorized para '%s' — verifica APIFY_API_KEY. "
                        "headers=%s body=%s",
                        query, relevant_headers, body_snippet,
                    )
                elif status == 403:
                    log.error(
                        "Apify HTTP 403 Forbidden para '%s' — token sin permisos o actor privado. "
                        "headers=%s body=%s",
                        query, relevant_headers, body_snippet,
                    )
                elif status == 429:
                    ...
                    log.warning(
                        "Apify HTTP 429 Too Many Requests para '%s' — cuota/rate-limit superado. "
                        "retry-after=%s headers=%s body=%s",
                        query, retry_after, relevant_headers, body_snippet,
                    )
                else:
                    log.error(
                        "Apify HTTP %d para '%s'. headers=%s body=%s",
                        status, query, relevant_headers, body_snippet,
                    )
            ...
            try:
                ...
                log.error(
                    ...
                    query, json_err, utils.trunc(resp.text, 2000),
                )
                return []

        if not isinstance(items, list):
            log.error(
                "Apify returned invalid JSON para '%s': esperaba list, recibió %s — body=%s",
                query, type(items).__name__, utils.trunc(str(items), 2000),
            )
            return []
